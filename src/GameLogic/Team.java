...
public abstract class Team {
     ...
     * @param enemyTeam the enemy team of this team
     * @return a boolean representing if the action was successful or not
     * @throws Exception
     */
    public abstract boolean performAction(ChessAction action, ChessBoard board, Team enemyTeam) throws Exception;
     ...
     *
     */
    protected void getKingLocation() {
        for(Piece piece : pieces) {
            if(piece instanceof King && piece.getTeam().equals(this)) {
     ...
     */
    public ArrayList<Piece> getMovablePieces(ChessBoard board, PlayerTeam otherTeam) {
    ...
    protected ChessAction getBestMove(Team team, ChessBoard board) {
        team.findAllAvailableMoves(board);
        team.removeIntoCheckMoves(board, this);
        Random rand = new Random();
        ChessAction bestAction = team.getMoves().get(rand.nextInt(team.getMoves().size()));
        int currentBestValue = 0;
        for(ChessAction action : team.getMoves()) {
            if(action instanceof Capture) {
                if(determinePieceValue(action.getEndLocation(), board) > currentBestValue) {
                    bestAction = action;
                }
            }
        }
    ...
    protected int determinePieceValue(Location location, ChessBoard board) {
